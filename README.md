# Blast Maze

ブラウザだけで遊べる、ボンバーマン風の Canvas ゲームです。

[プレイはこちら](https://bekuta-jp.github.io/blast-maze/)

現在のバージョン: `v1.8`

## 遊び方

- 矢印キーまたは WASD: 移動
- Space または Enter: 爆弾を置く
- 敵を全て倒し、ブロックに隠れた出口に入ると次のステージへ進みます
- 黄色いアイテムで同時設置数、赤いアイテムで爆風火力が上がります
- プレイ中に Escape で中断できます
- ゲームオーバー時のパスワードを入力すると、そのステージからコンティニューできます

## 起動

`index.html` をブラウザで開くだけで起動します。

ローカルサーバーで確認したい場合:

```sh
python3 -m http.server 8080
```

その後 `http://localhost:8080` を開いてください。

## GitHub Pages

`.github/workflows/pages.yml` で GitHub Pages へデプロイできます。

1. GitHub のリポジトリ設定で Pages の Source を `GitHub Actions` にします
2. `main` ブランチへ push します
3. Actions の `Deploy to GitHub Pages` が完了すると公開 URL が表示されます

Chrome では公開 URL を開いたあと、アドレスバーまたはメニューから Web アプリとしてインストールできます。

## 更新履歴

<details>
<summary>開く</summary>

### v1.8 - 2026-04-23

- ステージクリア時はパワーアップを維持
- コンティニュー時はパワーアップを初期化
- Escape でプレイを中断できるよう変更
- 爆弾数アップと火力アップのアイテムを分離
- 出現したアイテムに爆風が当たると消失

### v1.7 - 2026-04-23

- 被弾時にステージ状態とパワーアップを維持
- 現在の爆弾数と火力をHUDに表示
- ステージ番号の短いハッシュによるパスワードコンティニューを追加

### v1.6.1 - 2026-04-22

- ゲームオーバー時の再挑戦ボタン表記を統一

### v1.6 - 2026-04-22

- 壁やボムにぶつかって動けない時の効果音を追加

### v1.5.5 - 2026-04-22

- ポップアップを閉じられない問題を修正

### v1.5.4 - 2026-04-22

- Web版でモーダルが開かない問題に対するJS補助とキャッシュ対策を追加

### v1.5.3 - 2026-04-22

- モーダル開閉のクリック処理競合を修正

### v1.5.2 - 2026-04-22

- URLハッシュに依存しないポップアップ方式に変更
- 旧ハッシュが残っている場合は起動時に自動クリア

### v1.5.1 - 2026-04-22

- 操作説明と履歴をCSSだけでも開ける方式に変更

### v1.5 - 2026-04-22

- 画面上部に操作説明ボタンを追加

### v1.4 - 2026-04-22

- 履歴ポップアップを通常のオーバーレイに変更して安定化
- スマホ操作ボタンをX字に区切った正方形D-padへ再設計
- 一般的なスマホ画面で1画面に収まるようレイアウトを調整

### v1.3 - 2026-04-22

- スマホ操作ボタンの文字選択対策を追加
- 更新履歴を画面上部ボタンから開くポップアップに変更

### v1.2 - 2026-04-22

- 効果音を追加
- Chrome でインストールできる Web アプリ設定を追加

### v1.1.2 - 2026-04-22

- タイトル表示を Blast Maze に統一

### v1.1.1 - 2026-04-22

- プレイヤー移動を約1/8マスごとの段階移動に調整

### v1.1 - 2026-04-22

- スマホ表示で十字移動を左、ボムボタンを右へ配置

### v1.0 - 2026-04-22

- 初回リリース
- 移動、爆弾、敵、出口、パワーアップ、ステージ進行を追加
- GitHub Pages デプロイに対応

</details>
